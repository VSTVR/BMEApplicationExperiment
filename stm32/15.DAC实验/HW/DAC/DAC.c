...
static  void ConfigDAC1(void)
{
  ...
  GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_AIN;         //设置引脚的模式，模拟输入 (￥节能)
  ...
  DAC_SetChannel1Data(DAC_Align_12b_R, 0);        //12位右对齐数据格式设置DAC值，（￥在DAC固件库里）
...
{  
  ...
  DMA_DeInit(DMA2_Channel3);                 //DMA2通道3
  ...
  DMA_Init(DMA2_Channel3, &DMA_InitStructure);                          //根据参数初始化DMA2通道3
...
{
  if(DMA_GetITStatus(DMA2_IT_TC3))                //中断标志（￥在dma固件库里）
  {
    NVIC_ClearPendingIRQ(DMA2_Channel3_IRQn);     //清除标志
    DMA_ClearITPendingBit(DMA2_IT_GL3);           //清除全部中断标志  （￥同上）      
...
void InitDAC(void)
{              
  //（task）s_strDAC1WaveBuf.waveBufAddr  = (u32)GetSineWave100PointAddr(); //波形地址（￥这里只发送正弦波）
