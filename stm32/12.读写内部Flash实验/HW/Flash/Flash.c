...
*********************************************************************************************************/
//大容量芯片为 2K 一个 PAGE 
#define FLASH_PAGE_SIZE ((u16)0x0800)     //十进制为 2048 
/*********************************************************************************************************
*                                              内部变量
*********************************************************************************************************/
 static u32 s_arrFlashBuf[FLASH_PAGE_SIZE / 4];  //最多是 2K 字节 
/*********************************************************************************************************
*                                              内部函数声明
*********************************************************************************************************/
static u32  ReadWord(const u32 addr);           //读取指定地址的字（字，即 32 位) 

//不检查的写入字（字，即 32 位）格式的数据 
...
/*********************************************************************************************************
* 函数名称: ReadWord 
* 函数功能: 读取指定地址的字（字，即 32 位)
* 输入参数: addr：读地址（此地址为 4 的倍数）
* 输出参数: 
* 返 回 值: 
* 创建日期: 2018年01月01日
* 注    意:
...
* 函数功能: 不检查的写入字（字，即 32 位)格式的数据
* 输入参数: startAddr：起始地址（由于是字（32 位）写入，因此，此地址必须为 4 的倍数）；pBuf：数据指针 
...
{
  u16 i;     u32 pagePos;              //扇区地址，即起始地址 startAddr 所在的扇区地址 
  u16 pageOff;              //扇区内偏移地址（32 位计算），即起始地址 startAddr 所在的扇区的偏移地址 
  u16 pageResidue;          //扇区内剩余空间大小（32 位计算） 
  u32 offAddr;              //去掉 0X08000000 后的地址 
  u32 addr;                 //写地址，字（32 位）为单位 
  u16 numToWriteResidue;    //待写入的字（32 位）的剩余数
  
  addr = startAddr;         //将起始地址赋给 addr，addr 在 Flash 进行写的时候递增 
  numToWriteResidue = numToWrite;   //numToWrite 是形参，将其复制给动态改变的 numToWriteResidue

  if(addr < STM32_FLASH_BASE || (addr >= (STM32_FLASH_BASE + 1024 * 256))) 
  {
    printf("error\r\n"); 
    return;           //如果起始地址小于 Flash 的基地址，或者起始地址超过 Flash 的范围，即为非法地址，直接退出 
  }
  
  FLASH_Unlock();     //解锁 
  
  offAddr     = addr - STM32_FLASH_BASE;          //实际偏移地址，即起始地址 startAddr 在 Flash 中的偏移地址 
  pagePos     = offAddr / FLASH_PAGE_SIZE;        //扇区地址 
  pageOff     = (offAddr % FLASH_PAGE_SIZE) / 4;  //在扇区内的偏移地址（32 位计算） 
  pageResidue = FLASH_PAGE_SIZE / 4 - pageOff;    //扇区内剩余空间大小（32 位计算）
  ...
  while(1)
  {
    ...
    {
      if(s_arrFlashBuf[pageOff + i] != 0XFFFFFFFF)//判断待写入的区域是否已经被擦除，为 0xFFFFFFFF 则已经被擦除 
      {
        break;                                    //如果待写入的区域有数据，需要擦除，则跳出 for 循环 
      }
    }
    ...
    {
      pagePos++;                    //扇区地址增 1，即转移到相邻的下一个扇区 
      pageOff = 0;                  //偏移位置为 0 
      pBuf += pageResidue;          //指针偏移，字（32 位）为单位 
      addr += pageResidue * 4;      //扇区内写地址，即转移到相邻的下一个扇区的起始地址，字（32 位）为单位 
      numToWriteResidue -= pageResidue;               //减去已经写入的字数
      
      if(numToWriteResidue > (FLASH_PAGE_SIZE / 4))   //除以 4 是因为是以字（32 位）为单位 
      {
...
* 输入参数: startAddr：起始地址（由于是字（32 位）读取，因此，此地址必须为 4 的倍数）；pBuf：数据指针 
*           numToRead：字（32 位）数，即要读取的32 位数据的个数
* 输出参数: 
* 返 回 值: 
* 创建日期: 2018年01月01日
* 注    意:
...
void  STM32FlashReadWord(const u32 startAddr, u32* pBuf, u16 numToRead)
{
  ...
  {
    pBuf[i] = ReadWord(addr);     //读取字（32 位） 
    addr += 4;                    //由于是字（32 位），故地址增加 4 
  }
}
