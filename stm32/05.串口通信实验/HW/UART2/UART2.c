...
static  u8  s_arrSendBuf[UART2_BUF_SIZE];    //发送串口BUF 
static  u8  s_arrRecBuf[UART2_BUF_SIZE];    //接收串口BUF
...
{ 
  ...
  InitQueue(&s_structUARTSendCirQue, s_arrSendBuf, UART2_BUF_SIZE); //为缓冲区创立队列
...
static  void  ConfigUART(u32 bound) 
 { 
   ...
   RCC_APB2PeriphClockCmd(RCC_APB1Periph_USART2, ENABLE);    //开启 USART 的时钟 
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);     //开启 USART 的 GPIO 时钟 //配置 USART.Tx 的 GPIO 
   //RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART1|RCC_APB2Periph_GPIOA, ENABLE);
 
//GPIO   
   GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_2;               //设置 USART.TX 的引脚 
   ...
   GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_3;              //设置 USART.RX 的引脚 
...
//USART   
   USART_StructInit(&USART_InitStructure);                   //初始化串口的各项配置 
    ...
    USART_Init(USART2, &USART_InitStructure);                 //根据参数初始化 USART //配置 USART 的 NVIC 
 
//NVIC 
   NVIC_InitStructure.NVIC_IRQChannel = USART2_IRQn;         //开启 USART 的中断 
    ...
    USART_ITConfig(USART2, USART_IT_TXE,  ENABLE);  //发送中断使能 
    USART_Cmd(USART2, ENABLE);                      //使能USART 
    
    s_iUARTTxSts = UART2_STATE_OFF;  //初始化USART.TX状态标志 
}
...
{ 
  s_iUARTTxSts = UART2_STATE_ON;                     //将s_iUARTTxSts的状态置为打开 ，注意这句必须放前面
  USART_ITConfig(USART2, USART_IT_TXE, ENABLE);     //发送中断使能 
}
...
{   
    USART_SendData(USART2, (u8)ch);//发送一个字节数据到串口 //等待发送完毕 
...
{ 
  ...
  if(wLen < UART2_BUF_SIZE) 
    { 
      if(s_iUARTTxSts == UART2_STATE_OFF) 
        { 
          EnableUARTTx(); 
        }     
